...
# credo che l'idea era che in initialize_defaults si dovevano impostare dei valori nelle singole istanze e non nella classe
    validates_presence_of :start_date_from,
      :if => lambda{|record|
        !record.invalid_start_date?                               &&
        record.start_date_spec?                                   &&
        record.start_date_spec.is_in?(self.class.start_date_spec_values)     &&
        record.start_date_valid?                                  &&
        record.start_date_valid.is_in?(self.class.start_date_valid_values)   &&
        record.start_date_format?                                 &&
        record.start_date_format.is_in?(self.class.start_date_format_values)
      }
# Upgrade 2.0.0 fine
...
=end
    validates_presence_of :end_date_from,
      :if => lambda{|record|
        record.start_date_from?                                       &&
        record.end_date_format?                                       &&
        record.end_date_format.is_in?(self.class.end_date_format_values - ['U'])
      }
# Upgrade 2.0.0 fine
...
=end
    scope :ordered, -> { {:order => "#{self.table_name}.order_date ASC"} }
    scope :for_entity, ->(entity) {
      where(["#{self.table_name}.#{entity_foreign_key} = ?", entity.id])
    }
# Upgrade 2.0.0 fine
