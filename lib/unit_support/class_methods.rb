...
=end
        select(select_for_grid(selected_attributes)).
        joins(joins_for_grid).
        where(conditions_for_grid(params)).
        order(order_sql_for_grid(params)).
        limit((params[:rows] || Unit.per_page)).
        page(params[:page])
# Upgrade 2.0.0 fine
    end
...
=end
    def bulk_create(number_of_records, params, opts={})
      ...
      transaction do
        bulk_insert_builder(number_of_records, params, opts).statements.each do |statement|
          ar_connection.execute(statement)
        end

        # NOTE: NON THREAD-SAFE, per renderlo thread-safe, gestire il last come variabile di istanza dell'oggetto BulkInsertBuilder
        result              = ar_connection.execute(last_insert_id_sql)
        first_inserted      = find(first_inserted_id(result, number_of_records))
# Upgrade 2.0.0 inizio
#        result.free if ar_connection.adapter_name.downcase.match(/mysql/)
        if ar_connection.adapter_name.downcase == "mysql" then result.free end
# Upgrade 2.0.0 fine
        new_units_count     = Unit.where({:fond_id => params[:fond_id]}).count('id')

        # TODO: [Luca] spostare in un metodo
        Fond.where({:id => params[:fond_id]}).update_all({:units_count => new_units_count})
        first_inserted.update_sequence if first_inserted.respond_to? :update_sequence
        first_inserted.update_sequence_for_structural_root if first_inserted.respond_to? :update_sequence_for_structural_root
      end
...
=end
        fond_ids =  select("DISTINCT fond_id").where(id: unit_ids).
                    map{|u|u.fond_id} # warning: must be found *before* deletion
# Upgrade 2.0.0 fine
...
#      when 'mysql', 'mysql2' then %Q{SELECT LAST_INSERT_ID() AS `id`}
      case ar_connection.adapter_name.downcase
      when 'mysql', 'mysql2' then %Q{SELECT LAST_INSERT_ID() AS id}
# Upgrade 2.0.0 fine
...
#      end
      case ar_connection.adapter_name.downcase
        when 'mysql' then result.fetch_hash['id']
        when 'mysql2' then result.first[0].to_i
        when 'sqlite', 'postgresql' then result.first['id'].to_i - number_of_records.to_i + 1
      end
# Upgrade 2.0.0 fine
    end
...
# Upgrade 2.0.0 inizio
#        types.map(&:to_s).include?(column.instance_variable_get(:'@type').to_s) && columns_to_search.include?(column.name)
        types.map(&:to_s).include?(column.type.to_s) && columns_to_search.include?(column.name)
# Upgrade 2.0.0 fine
      end
    end
