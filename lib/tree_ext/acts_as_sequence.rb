...
=end
      ...
      scope :prepare_ordered_subtree, -> {
        select(select_for_tree_options).order(order_for_tree_options)
      }
      scope :active, -> {where({:trashed => false})}
      scope :trashed, -> {where({:trashed => true})}
      scope :trashed_roots, -> {where({:trashed => true, :trashed_ancestor_id => nil})}
# Upgrade 2.0.0 fine
