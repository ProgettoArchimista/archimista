...
=end
        ...
        self.class.
          ...
          joins([:fond]).
          order("fonds.sequence_number, units.sequence_number").
          where("units.fond_id IN (#{structural_root.subtree_ids.join(', ')})
                            AND fonds.sequence_number < #{structural_parent.sequence_number}").
          last
# Upgrade 2.0.0 fine
      end
...
=end
        ...
        self.class.
          select("id, ancestry, sequence_number").
          where("fond_id = #{fond_id} AND ancestry_depth = #{ancestry_depth} AND position < #{position}").
          order("position").
          last.
          subtree.
          select("id, ancestry, sequence_number").
