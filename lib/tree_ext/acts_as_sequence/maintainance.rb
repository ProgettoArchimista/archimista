...
#        Unit.update_all(["position = ? AND root_fond_id = ?", nil, root_id], :fond_id => subtree_ids)
        Unit.where(:fond_id => subtree_ids).update_all(["position = ? AND root_fond_id = ?", nil, root_id])
# Upgrade 2.0.0 fine
...
=end
          node.units.roots.order('legacy_position').each do |root_unit|
            root_unit.update_attribute(:position, root_unit_position)
            root_unit_position += 1
            subunit_position = 1
            root_unit.children.order('legacy_position').each do |subunit|
              subunit.update_attribute(:position, subunit_position)
              subunit_position  += 1
            end
          end
