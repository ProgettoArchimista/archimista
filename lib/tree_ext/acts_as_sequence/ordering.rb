...
#          nodes = subtree.active.find(:all, :select => "id, sequence_number", :order => "sequence_number")
          nodes = subtree.active.select("id, sequence_number").order("sequence_number")
# Upgrade 2.0.0 fine
...
=end
        @other_ordered_active_node_ids ||=
          root.subtree.active.
          select('id').
          where(["id NOT IN (?)", ordered_active_subtree_ids]).
          order("sequence_number").
          map{|node| node.id}
# Upgrade 2.0.0 fine
      end
...
#          find(:last, :select => "id, ancestry, sequence_number", :order => "sequence_number")
          select("id, ancestry, sequence_number").order("sequence_number").last
# Upgrade 2.0.0 fine
        else
          parent
        end
...
=end
        @preceding_active_sibling ||=
          siblings.active.
          select("id, ancestry, sequence_number").find_by("position = ?", (position-1))
# Upgrade 2.0.0 fine
      end

      def active_tree_ids
# Upgrade 2.0.0 inizio
#        @active_tree_ids ||= root.subtree.active.find(:all, :select => "id").map{|node| node.id}
        @active_tree_ids ||= root.subtree.active.select("id").map{|node| node.id}
# Upgrade 2.0.0 fine
      end

      def active_subtree_ids
# Upgrade 2.0.0 inizio
#        @active_subtree_ids ||= subtree.active.find(:all, :select => "id").map{|node| node.id}
        @active_subtree_ids ||= subtree.active.select("id").map{|node| node.id}
# Upgrade 2.0.0 fine
      end
...
#        root.subtree.active.find(:all, :select => 'id', :order => "sequence_number").map{|node| node.id}
        root.subtree.active.select('id').order("sequence_number").map{|node| node.id}
# Upgrade 2.0.0 fine
      end

      def all_external_node_ids
# Upgrade 2.0.0 inizio
#        external_nodes_class.find(:all, :select => "id", :conditions => {:fond_id => root.subtree_ids}).map{|i|i.id}
        external_nodes_class.select("id").where(fond_id: root.subtree_ids).map{|i|i.id}
# Upgrade 2.0.0 fine
      end
