...
=end
        initialize_sequence_ids(
          :all => external_nodes_class.select("id").where(fond_id: subtree_ids).map{|i|i.id},
          :active => flat_ordered_external_nodes(:active_only => true, :ids_only => true) )
# Upgrade 2.0.0 fine
        self.values_for_rearrangement = values_for_sequence
...
#        self.class.update_all("sequence_number = 1", {:id => id}) if id.present? && is_root?
        self.class.where({:id => id}).update_all("sequence_number = 1") if id.present? && is_root?
# Upgrade 2.0.0 fine
      end
...
=end
      def update_sequence_in_new_node
        if id.present? && !is_root? && root_id.present?
          self.class.
          where("sequence_number >= #{new_position_in_sequence} AND ancestry LIKE '#{root_id}%'").
          update_all("sequence_number = sequence_number+1")

          self.class.where({:id => id}).update_all("sequence_number = #{new_position_in_sequence}")
        end
      end
# Upgrade 2.0.0 fine
...
=end
          tmp_table_bulk_inserts.each { |bulk_insert| self.class.connection.execute(bulk_insert) }
          transaction do
            self.class.connection.execute(nodes_rearrangement_sql(given_table_name, rank_column))
          end
# Upgrade 2.0.0 fine
        end
      end
