...
#    first_created = Unit.bulk_create( params[:number_of_rows], params[:unit], :position_scope => [:fond_id, :ancestry], :sequence_number_scope => :fond_id )
    first_created = Unit.bulk_create( params[:number_of_rows], unit_params, :position_scope => [:fond_id, :ancestry], :sequence_number_scope => :fond_id )
# Upgrade 2.0.0 fine
...
#      new_fond_name = Fond.find(params[:new_fond_id], :select => "name").name
      new_fond_name = Fond.select("name").find(params[:new_fond_id]).name
# Upgrade 2.0.0 fine
...
=end
    @unit = Unit.find(params[:id])
    @siblings = @unit.siblings.where("fond_id = #{@unit.fond_id}").order(:position).to_a.delete_if{|e| e.id == @unit.id}

    @display_sequence_numbers = Unit.display_sequence_numbers_of(@unit.fond.root)
    render :partial => 'units/move', :locals => {:units => @siblings}, :object => @unit, :layout => false, :as => "object"
# Upgrade 2.0.0 fine
  end
...
# Upgrade 2.0.0 inizio
#      @unit.position = Unit.prepare_position_by('position DESC').first(:conditions => {:ancestry => @parent.ancestry, :root_fond_id => @unit.fond.root.id}).position + 1
      @unit.position = Unit.prepare_position_by('position DESC').where({:ancestry => @parent.ancestry, :root_fond_id => @unit.fond.root.id}).first.position + 1
# Upgrade 2.0.0 fine
      @unit.save!
...
# Upgrade 2.0.0 inizio
#      @unit.position = Unit.prepare_position_by('position DESC').first(:conditions => {:ancestry => @new_parent.ancestry, :root_fond_id => @unit.fond.root.id}).position + 1
      @unit.position = Unit.prepare_position_by('position DESC').where({:ancestry => @new_parent.ancestry, :root_fond_id => @unit.fond.root.id}).first.position + 1
# Upgrade 2.0.0 fine
      @unit.save!
...
#    @unit = Unit.find(params[:id], :include => :events)
    @unit = Unit.includes(:events).find(params[:id])
# Upgrade 2.0.0 fine
# Upgrade 2.0.0 inizio Strong parameters
...
=end
    @iccd_terms_oa_mtc = IccdTermsOaMtc.select("distinct mtc as value").
      where(["LOWER(mtc) LIKE ?", "#{term}%"]).
      order('value').
      limit(10)
# Upgrade 2.0.0 fine
    respond_to do |format|
...
=end
    @iccd_terms_oa_ogtd = IccdTermsOaOgtd.select("ogtd, ogtt").
      where(["LOWER(ogtd) LIKE ?", "#{term}%"]).
      order('ogtd, ogtt').
      limit(10)
# Upgrade 2.0.0 fine
...
=end
    @iccd_terms_bdm_ogtd = IccdTermsBdmOgtd.select("ogtd as value").
      where(["LOWER(ogtd) LIKE ?", "#{term}%"]).
      order('ogtd').
      limit(10)
# Upgrade 2.0.0 fine
...
=end
    @iccd_terms_bdm_mtct = IccdTermsBdmMtct.select("mtct as value").
      where(["LOWER(mtct) LIKE ?", "#{term}%"]).
      order('mtct').
      limit(10)
# Upgrade 2.0.0 fine
    ActiveRecord::Base.include_root_in_json = false
...
=end
    @iccd_terms_bdm_mtcm = IccdTermsBdmMtcm.select("mtcm as value").
      where(["LOWER(mtcm) LIKE ?", "#{term}%"]).
      order('mtcm').
      limit(10)
# Upgrade 2.0.0 fine
    ActiveRecord::Base.include_root_in_json = false
...
=end
    @unit = Unit.new(unit_params).tap do |unit|
      unit.created_by = current_user.id
      unit.updated_by = current_user.id
    end
# Upgrade 2.0.0 fine
...
#    attribute, value = *params[:unit].to_a.flatten[0..1]
    attribute, value = *unit_params.to_a.flatten[0..1]

#    Unit.update_all(["#{attribute} = ?", value], {:id => params[:id]})
    Unit.where({:id => params[:id]}).update_all(["#{attribute} = ?", value])
# Upgrade 2.0.0 fine
